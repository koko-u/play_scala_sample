...
with BeforeAndAfterEach {
  ...
  it should "create and retrieve a User" in {
    ...
    val bob = User.find("email={email}")
                  .on("email" -> "bob@gmail.com")
                  .first()
  ...
  it should "connect a User" in {
    ...
    User.connect("bob@gmail.com", "badpass") should be (None)
  ...
  it should "support comments" in {
    User.create(User(Id(1), "bob@gmail.com", "secret", "Bob", false))
    Post.create(Post(Id(1), "My first post", "Hello!", new Date(), 1))
